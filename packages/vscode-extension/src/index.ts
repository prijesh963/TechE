...
import {
  createDashboardHtml as renderDashboardHtml,
  formatSession,
  formatAgentInsights,
...
// Re-exported unchanged from @copilot-architect/dashboard: this package used
// to define them directly, and other modules (and every existing test) still
// reach them through this module's own surface.
export {
  formatSession,
  formatAgentInsights,
  loadDashboardSession,
...
// ExtensionState, DashboardSession, SessionActivityInsights, DashboardArtifacts
// and ContextInsights moved to @copilot-architect/dashboard — a second shell
// (the IntelliJ plugin, via the CLI's `dashboard` command) needed the exact
// same render+load logic, so it now lives where both can import it. See that
// package for the full docs on each type.
...
/**
 * Render+load logic moved to `@copilot-architect/dashboard` — see that
 * package for `formatSession`, `formatAgentInsights`, `loadDashboardSession`,
 * `loadDashboardArtifacts` and the dashboard types, all re-exported below
 * unchanged so nothing else in this file (or its tests) had to move with it.
 *
 * `createDashboardHtml` alone gets a thin wrapper: it is the one seam where
 * VS Code's own `command:` URI scheme has to be assembled, which the shared
 * renderer deliberately knows nothing about (see `CreateDashboardHtmlOptions`
 * in that package). Same single-argument signature as before, same output.
 */
export function createDashboardHtml(state: ExtensionState): string {
  const actionsHtml = `${DASHBOARD_PRIMARY_ACTIONS.map(renderActionLink).join("")}<a href="command:copilotArchitect.moreActions">More actions…</a>`;
  return renderDashboardHtml(
    { ...state, buildVersion: state.buildVersion ?? EXTENSION_VERSION },
    { actionsHtml }
  );
}
