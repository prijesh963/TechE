...
| The pattern-based symbol extractor capped every file at 100 symbols before storage, so a real method declared past that point never entered the index and grounding reported it as fabricated no matter how the check was framed | pre-phase | Cap raised to 2000 in `extractSymbolsByPattern`; a hand-written file exceeding that is still a safety valve against a minified/generated one, not the expected case |
